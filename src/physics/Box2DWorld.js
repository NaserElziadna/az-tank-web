...
export class Box2DWorld {
  ...
  /** Dynamic tank body. The COLLISION shape is a circle (radius = COLLISION_RADIUS)
   *  even though the tank renders as a box: a circle slides smoothly along walls
   *  and never catches on corners, so tanks "move normally and slide on contact"
   *  instead of wedging. It also matches the radius the AI's aim already assumes. */
  createTank(gameObject, x, y, rotation) {
    ...
    fd.friction = 0.05;
    fd.restitution = 0;
    fd.shape = new b2CircleShape(C.TANK.COLLISION_RADIUS);
